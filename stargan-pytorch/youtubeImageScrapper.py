...
driver = webdriver.Chrome("/home/abhiyush/Downloads/chromedriver_linux64/chromedriver")

def get_urls(text, limit=1):
...
def download_video(url, path="/video", max_duration=10):
...
# get_urls('Freinds and Big bang theory')
extract_images_from_word("Friends and Big bang theory")
...
url = "https://www.youtube.com/watch?v=kObFJLCucPA"
download_video(url, path="/video", max_duration=10)
...
driver = webdriver.Chrome("/home/abhiyush/Downloads/chromedriver_linux64/chromedriver",options=option)
text ="money heist"
query = urllib.parse.quote(text)
url = "https://www.youtube.com/results?search_query=" + query
driver.get(url)
...
url = urls[0]
path="/media/abhiyush/New Volume/Macquarie/master_of_data_science/semester_3(session_2_2020)/COMP8240-Application_of_Data_Science/project/COMP8240_Major_project_Group_A/stargan-pytorch/video"
try:
    yt = YouTube(url)
    # duration = int(yt.player_config_args['player_response']['streamingData']['formats'][0]['approxDurationMs'])
    # if duration < max_duration * 60 * 1000:
